...
# # The new tissue specific score between two nodes is A+B+C, this is the new value to replace shortest path.
# @memo_2
# def drugs_shortest_path_2(G, source_id, target_id, weight):
#     if weight == True:
...
# @memo_2
# def drugs_shortest_path(G, node1, node2):
...
class Sets_Lengths:
    # TODO: DELETE UNNESSARSAY DISTANCE
...
class Network_Distance:
    ...
    def cal_shortest(self):
        ...
        """
        # TODO: change the way to get sum value
        values_outer = []
        for node_from in self.nodes_from:
    ...
    def cal_z_score(self, distance_method, random_time, network, seed):
        ...
        random_d_s = []
        for i in tqdm.tqdm(range(random_time)):
...
def closest_distance(G, A, B):
    dis_pd = pd.DataFrame(columns=A, index=B)
    for node1 in A:
        for node2 in B:
            try:
                path = nx.shortest_path(G, node1, node2, weight='w')
                distance = sum([G[path[i]][path[i + 1]]['w'] for i in range(len(path) - 1)])
            except nx.NetworkXNoPath:
                distance = None
    dis_pd[node1, node2] = distance
    return dis_pd
